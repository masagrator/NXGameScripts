...
def ReplaceChara(string):
	fromthis = ["«", "»", "––", "——", "~", ":", "á", "é", "í", "ó", "ú", "ñ", "ß", "ï", "Ä", "ä", "Ö", "ö", "Ü", "ü", "¿", "¡"]
	tothis = ["≪", "≫", "㊤㊦", "㊤㊥", "〜", "：", "§", "☆", "★", "○", "●", "◎", "◇", "◆", "□", "■", "△", "▲", "▽", "▼", "※", "〒"]
...
def load_font_metrics(json_path):
	"""
	Loads the font metrics from the updated JSON dump and maps 
	each character to its WIDTH.
	"""
	if not os.path.exists(json_path):
		raise FileNotFoundError(f"Could not find the file: {json_path}")
	...
	for entry_id, info in data.items():
		...
		if glyph is not None and width is not None:
			metrics[glyph] = width

	return metrics

def calculate_text_width(text, metrics, letter_spacing=0, default_width=24):
	"""
	Calculates the total pixel width of a string.
	"""
	total_width = 0
	
	for i, char in enumerate(text):
		# Retrieve width from our mapping
		char_width = metrics.get(char, default_width)
		
		# If char not found, you might want to log it
		if char not in metrics:
			print(f"Warning: Character '{char}' not found. Using default width {default_width}.")
...
def generateCommand(Dict, entry_number, untranslated):
	try:
		Dict["TYPE"]
	except:
		entry = []
		for i in range(0, len(Dict["STRINGS"])):
			if (untranslated == False):
				if (i > 0):
					generateCommand.line_width = 0
				text = ReplaceChara(Dict["STRINGS"][i])
				entry_width = calculate_text_width(text, glyph_widths, letter_spacing=0)
				if (entry_width > max_width):
					print("Detected line overflow. Entry number: %d, line: %d. Got width: %d, Max allowed: %d" % (entry_number, i+1, entry_width, max_width))
					print("Entry dump:")
					print(text)
					print("Adding break line is not possible.")
					sys.exit()
				generateCommand.line_width += entry_width
				if (generateCommand.line_width > max_width):
					print("Detected line overflow. Entry number: %d, line: %d. Got width: %d, Max allowed: %d" % (entry_number, i+1, generateCommand.line_width, max_width))
					print("Entry dump:")
					print(text)
...
new_file = open("database/story.dat", "wb")
...
glyph_widths = load_font_metrics("font/dump.json")
...
for i in range(0, len(order)):
	...
	for x in range(0, len(DUMP)):
		data = generateCommand(DUMP[x], x, untranslated)
#        size += len(data)
