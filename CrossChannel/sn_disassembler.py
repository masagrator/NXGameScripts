...
for i in range(len(files) - 1):
	...
	while (file.tell() < end_pos):
		...
		match(cmd):
			...
			case 0x14:
				entry["CMD"] = "%X" % cmd
				DATA = file.read(0x4).hex() # Always "98800200"
			# case 0x15:
			# 	entry["CMD"] = "%X" % cmd
			# 	entry["DATA"] = file.read(0x4).hex()
			# case 0x16:
			...
			case 0x31:
				entry["CMD"] = "%X" % cmd
				DATA = file.read(0x2).hex() # Always "8080"
				count = int.from_bytes(file.read(0x2), byteorder="little")
			...
			case 0x3A:
				entry["CMD"] = "%X" % cmd
				DATA = file.read(0x4).hex() # Always "00000000"
			case 0x3B:
				entry["CMD"] = "%X" % cmd
				entry["DATA"] = file.read(0x2).hex()
			case 0x3C:
			...
			case 0x48:
				entry["CMD"] = "%X" % cmd
				DATA = file.read(0x2).hex() # Always "FFFF"
			case 0x49:
				entry["CMD"] = "%X" % cmd
				DATA = file.read(0x4).hex() # Always "FFFFFFFF"
			case 0x4A:
				entry["CMD"] = "KEY_WAIT"
				DATA = file.read(0x2).hex() # Always "FFFF"
			case 0x4B:
			...
			case 0x7B:
				entry["CMD"] = "%X" % cmd
				DATA = file.read(0x4).hex() # Always "00002c01"
			# case 0x7C:
			# 	entry["CMD"] = "%X" % cmd
			# 	entry["DATA"] = file.read(0x4).hex()
			# case 0x7D:
			...
			case 0x83:
				entry["CMD"] = "%X" % cmd
				DATA = file.read(0x4).hex() # Always "ff800300"
			case 0x84:
				entry["CMD"] = "%X" % cmd
