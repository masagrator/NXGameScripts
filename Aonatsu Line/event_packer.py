...
for i in range(len(DUMP["ENTRIES"])):
    entries.append(DUMP["ENTRIES"][i]["FLAG1"].to_bytes(4, "little"))
    ...
    entries.append(DUMP["ENTRIES"][i]["FLAG2"].to_bytes(4, "little"))
    entries.append(0x3.to_bytes(4, "little"))
    entries.append(DUMP["ENTRIES"][i]["ID"].to_bytes(32, "little", signed=True))

os.makedirs("new", exist_ok=True)
new_file = open("new/event.datu8", "wb")
