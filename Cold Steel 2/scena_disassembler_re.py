...
def CalcGoto(file, entry, end):
	...
	entry2 = {}
	control = int.from_bytes(file.read(1), byteorder="little")
	passing = [2, 3, 4, 5, 6, 7, 8, 9, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF, 0x10, 0x11, 0x12, 0x13, 0x17, 0x18, 0x22]
	while (control != 1):
		entry2 = {}
...
def GenerateCommand(cmd, file, end):
	...
	match(cmd):
		...
		case 0x4A:
			entry["TYPE"] = "0x4A"
			entry["CONTROL"] = int.from_bytes(file.read(1), byteorder="little")
			if (entry["CONTROL"] in [0x2E, 0x36, 0x43, 0x41, 0x12, 0x2D, 0x10, 0x11, 0x2F, 0x13, 0x1E, 0x31, 0x2A, 0x25, 0xF, 0x1D]):
				return entry
...
for y in range(0, len(files)):
	...
	for i in range(0, function_names_count):
		...
		while(file.tell() < end):
			if ((FUNCTIONS_NAMES[i] != "") and (FUNCTIONS_NAMES[i] != "ShinigPomBtlset")):
				cmd = int.from_bytes(file.read(1), byteorder="little")
